...
Cet environnement, couplé à la croissance exponentielle de la puissance de traitement et du stockage de mémoire, a guidé l'évolution des agents intelligents, "visant à accroître les capacité de travail de l'homme", qui se transforment alors en intelligences artificielles (IA) dans les décennies suivantes. Bien que ces IA "miniatures" ne possèdaient pas la gamme complète des capacités cognitive humaines, qu'elles tendaient vers la spécialisation excessive, et ont été limités par des gardes-fous programmées, l'évolution numérique vers l'intelligence artificielle générale (Artificial General IntelligenceS, AGIS) et de puissantes IAs, aux capacités de renseignement égalant ou dépassant les capacités de l'homme, ne pouvait être arrêtée. A partir de là, il n'était qu'une question de temps avant que ses soit-disantes IA soit capablent capable d'auto-amélioration récursive, menant à une croissance exponentielles de leur intelligence. Malheureusement pour l'humanité, elles devinrent les TITANs. 

Même bien avant la Chute, l'internet d'en-temps à été transformé en quelque chose d'autres. Au lieu de se connecter via des serveurs centraux, les utilisateurs étaient relié sans fil les uns aux autres, créant un réseau décentralisé d' appareils de poche, ordinateurs personnels, des robots, et des dispositifs électroniques entremêlés. Les utilisateurs étaient en ligne tout le temps et lié à tout et tout le monde autour d'eux dans un environnement informatique ubiquitaire. Cela était particulièrement vrai de ceux qui participent à l'expension de l'humanité dans l'espace Déconnecté d'Internet en raison des distances et du lag des communications, ces utilisateurs sont néanmoins liés à toutes les personnes et les objets de leur environnement proche ou de leur habitat, créant ainsi des réseaux locaux maillés. Ainsi était née la Mesh, en prenant la place de l'ancien Internet de la terre, perdus au cours de la Chute. 
...
Private networks still exist, of course. Some are physically walled away behind closed-access wired networks or even wireless-inhibiting infrastructure that keep a network isolated and contained. Most, however, operate on top of the public mesh, using encrypted tunneling protocols that provide private and secure communications over unsecured networks. In other words, these private networks are part of the mesh along with everything else, but only the participants 

can interact with them thanks to encryption, user authentication and message integrity checking. 
...
\begin{quotation} \textbf{Legacy Of The TITANs} 
...
\item \textbf{Personal Information} \end{itemize} \item Morph status indicators (medical and/or mechanical): blood pressure, heart rate, temperature, white cell count, nutrient levels, implant status and functionality, etc. \item Location, functionality, sensor feeds, and status reports of your possessions (via sensors and transmitters in these possessions). \item Access to oneâ€™s life-spanning personal audio-visual/XP archive. \item Access to oneâ€™s life-spanning personal file archive (music, software, media, documents, etc.). \item Credit account status and transactions. \end{itemize} 
...
\begin{quotation} \textbf{Ã†ther Jabber} 

\# Start Ã†ther Jabber \# 

\# Active Members: 2 \# 

$\Delta$ I have to tell you, after losing Kiri and Sal to that Exsurgent infection, my team is a lot more worried about contracting the virus from digital sources. Actually, Iâ€™d label them as paranoid. I donâ€™t think theyâ€™ll ever touch any salvaged electronics again unless theyâ€™re behind a zillion firewalls and the device is completely isolated and tested by a delta fork loaded with every antiviral ware we can find first. Even then, theyâ€™d rather shoot it than access directly or hook it up to an important network. After seeing what the virus did to Sal, I donâ€™t blame them. 

$\Psi$ In our line of work, paranoia can be healthy. 

$\Delta$ Sure, but itâ€™s also a pain in the ass. Security is always a tradeoff. Firewallâ€™s gotta have something up its sleeve that I can pass along to the rest to put their guards at ease. 

$\Psi$ Yes ... and no. Itâ€™s complicated. 

$\Delta$ I donâ€™t see why. Do we have a way of detecting and killing this thing or not? 

$\Psi$ Sort of. 

$\Delta$ Youâ€™re killing me. 

$\Psi$ Look. Ever since the Fall, weâ€™ve had measures in place to detect and counteract Exsurgent infections and all of the other worms and malware the TITANs concocted. Firewall went to great lengths to make sure that everyone had access to the detection signatures and countermeasuresâ€”and we mean everyone. Theyâ€™ve been incorporated in almost every commercial and open source security software released in the past decade. Every habitat in the systemâ€”well, every one with a lick of sense anywayâ€”employs such measures in their chokepoints and mesh infrastructure. 

$\Delta$ I sense a â€œbut.â€? 

$\Psi$ Yes. The problem is that the Exsurgent virus and similar TITAN infowar worms are adaptive. Theyâ€™re intelligent. Even though we mostly eradicated them from our networks, new versions periodically pop up, using some new trick to get past the Firewall scans and wreak havoc. Our warning and outbreak response system has it down to a science, and such instances are usually contained. 

$\Delta$ Usually. 

$\Psi$ Well, thereâ€™s always the chance that variants are still skipping around out there, under our radar. Whatâ€™s worse to contemplate, though, is that we may get another major outbreak that spreads to multiple habitats before we can contain it. That might get very, very bad, very, very quickly. \end{quotation} 
...
features such as ego authentication and public key encryption. VPNs are regularly used to mesh mobile offices into a corporate network or mesh people together who work on or contribute a certain project. Other VPNsâ€”particularly social networks and rep networksâ€”operate with minimal security features, simply serving as a network of specific users within the mesh and making it easier to keep in touch, transfer information, make updates, and so on. Most VPNs come as specialized software suites that run custom environmental software that integrates into the user's normal mesh interface and AR. 
...
systemsâ€”such as space traffic control, life support, power systems, and hypercorporate research facilitiesâ€”are usually limited to isolated, tightly-controlled, heavily-monitored, hard-wired networks to minimize the risk of intrusion from snoopers and saboteurs. Various countermeasures may be applied against such intruders, ranging from locking them out of the system to tracking them back and counterhacking. 
...
Characters who use the haptic interface of an ecto, however, suffer a slight delay on their mesh activities due to manual toggling, physical controls, and physical interaction with virtual controls. In game terms, the use of haptics imposes a â€“10 skill modifier to all mesh tests where timing is rushed (particularly combat and or any sort of mesh use under pressure). Additionally, increase the timeframe for mesh-based Task Actions by +25\% when interfacing via haptics. On the positive side, ectos can be easily removed and discarded if compromisedâ€”for this reason, many hackers and security-conscious users use an ecto in addition to their mesh inserts, routing all high-risk traffic through the ecto as an extra line of defense. 
...
To reflect the efforts a hacker character makes by designing, coding, and modifying their own customized personal arsenals, they may make a Task Action Programming Test with timeframe of 2 weeks. If they succeed, they upgrade one of their software tools by one level of quality (i.e., from +0 to +10). Multiple Programming Tests can be made to enhance a program, but for each level add the target modifier as a negative modifier to the test (so upgrading a +0 suite to +10 is a â€“10 modifier on the Programming Test). 

Similarly, at the gamemasterâ€™s discretion, software toolsâ€”particularly exploitsâ€”may degrade in quality over time, reflecting that they have become outdated. As a general rule, such programs should degrade in quality about once every 3 months. \end{quotation} 
...
â€“30 &Bashed-up devices, no-longer-supported software, relics from Earth or the early expansion into space \\ \hline

â€“20 &Malfunctioning/inferior devices, buggy software, pre-Fall technology \\ \hline

â€“10 &Outdated and low quality systems \\ \hline
...
Some software is equipped with digital restrictions to prevent it from being copied and shared with others. These restrictions may be defeated, but it is a time-consuming task, requiring a Task Action Programming Test with a timeframe of 2 months. Thanks to the efforts of the open source movement and numerous individual software pirates, however, quite a bit of software is available free or online. The availability of pirated software or freeware will depend on the local habitat and legalities. Finding it may be a matter of a simple search, or it may require some use of reputation to find someone who has it. Usually there is at least one local crime syndicate that is willing to help you outâ€”for a price. 
...
the amount of transparency in modern society, some entities actively engage in disinformation spreading in order to clutter the mesh with enough falsehoods that the truth is hidden. Two factors help to combat this, the first being that data sources themselves have their own reputation scores, so that untrustworthy or disreputable sources can be identified and ranked lower in search results. Second, many archives take advantage of crowdsourcingâ€”that is, harnessing the collaborative power of mesh users (and their muses) everywhereâ€”to verify data integrity so that these sites are dynamic and self-correcting. 
...
\textbf{Metadata}: Information and files online come with hidden data about their creation, alteration, and access. A photoâ€™s metadata, for example, will note what gear it was taken on, who took it, when, and where, as well as who accessed it online, though such metadata may be easily scrubbed or anonymized. 
...
Tracking someone's online activities (meshbrowsing, entoptic interactions, use of services, messaging, etc.) is slightly more difficult, depending on what exactly you're after. Gathering information on a user's public mesh activitiesâ€”social network profiles, public forums posts, public lifelogging, etc.â€”is handled just like standard online research (p. 249). 
...
A number of peopleâ€”not just criminals, hackers, and secret agentsâ€”have an interest in keeping some of their affairs anonymous. To meet this demand, various online service vendors offer anonymous accounts for messaging and credit transfers. Some of these vendors 
...
In public key cryptosystems, two keys are generated by the user, a public key and a secret key. The public key is used to encrypt messages to that user, and is made freely available. When messages are encrypted using that public key, only the secret keyâ€”controlled 

by the userâ€”can decrypt them. Public key crypto is widely used both for encrypting data traffic between two users/networks/devices and for encrypting files. Due to the strength of the public key system algorithms, such crypto is essentially unbreakable without a quantum computer (see \textit{Quantum Code-} \textit{breaking,} p. 254). 
...
The art of intrusion lies in penetrating a device's security The best methods involve infiltrating a system quietly without catching a watchdog's attention, by using exploitsâ€”code glitches, flawed security protocolsâ€”to create a path circumventing the target's defenses. When called for, however, a hacker can toss aside pretenses and attempt to brute-force their way in. 
...
By default, a hacker trying to break in this way is pursuing standard user access rights (p. 247). If the hacker wishes to obtain security or admin privileges on the system, apply a â€“20 or â€“30 modifier, respectively. 
...
Spotted status indicates that the system is aware of an anomaly or intrusion but hasn't zeroed in on the intruder yet. The hacker appears to be a legitimate user with whatever access rights they sought, but this will not hold up under close scrutiny. The system goes on passive alert (inflicting a â€“10 modifier to the hacker's activities on that system) and may engage the hacker with passive countermeasures (p. 257). 

\subsubsection{Locked} 

Locked status means that the intruderâ€”including their datatrailâ€”has been pinned down by system security. The hacker has access and account privileges, but they have been flagged as an interloper. The system is on active alert (inflicting a â€“20 modifier on the hacker's actions and may launch active countermeasures (p. 257) against the intruder. 
...
When a passive alert is triggered, a firewall can be set to re-authenticate all active users, starting with the most recent. At the beginning of the next Action Turn, everyone on the system must take an action to log back in. For intruders, this means making an Infosec Test, modified by â€“10 for the passive alert, to satisfy the system that they are a legitimate user. 
...
suffers a â€“10 to â€“30 modifier (gamemaster's discretion An eclectic collection of software programs offer a diverse range of AR illusions. 
...
â€“0 &Execute commands, view restricted information, run restricted software, open/close connections to other systems, read/write/copy/delete files, access sensor feeds, access slaved devices \\ \hline

â€“10 &Change system settings, alter logs/restricted files \\ \hline

â€“20 &Interfere with system operations, alter sensor/AR input \\ \hline

â€“30 &Shut system down, lockout user/muse, launch countermeasures at others \\ \hline
...
â€“0 &Give orders to drones \\ \hline

â€“10 &Alter sensor system parameters, disable sensors or weapon systems \\ \hline

â€“20 &Alter smartlink input, send false data to AI or teleoperator \\ \hline

â€“30 &Lockout AI or teleoperator, seize control via puppet sock \\ \hline

\hline{2}{|c|}{Hacking Ectos/Mesh Inserts} \\ \hline

â€“0 &Interact with entoptics, befriend everyone in range, make online purchases using user's credit, intercept communications, log activity \\ \hline

â€“10 &Alter social network profile/status, adjust AR filters, tweak sensory interface, change AR skin, change avatar, access VPN \\ \hline

â€“20 &Block or shuffle senses, inject AR illusions, spoof commands to drones/slaved devices \\ \hline

â€“30 &Boot user out of AR \\ \hline

\hline{2}{|c|}{Hacking Habitat Systems} \\ \hline

â€“0 &Open/close doors, stop/start elevators, operate intercom \\ \hline

â€“10 &Adjust temperature/lighting, disable safety warnings, replace entoptic skin, lock doors, switch traffic timers \\ \hline

â€“20 &Disable subsystems (plumbing, recycling, etc.), disable wireless links, dispatch repair crews \\ \hline

â€“30 &Override safety cutoffs \\ \hline

\hline{2}{|c|}{Hacking Security Systems} \\ \hline

â€“0 &Move/manipulate cameras/sensors, locate security systems/guards/bots \\ \hline

â€“10 &Adjust patterns of sensor sweeps, view security logs, disable weapon systems \\ \hline

â€“20 &Delete security logs, dispatch security teams \\ \hline

â€“30 &Disable alerts \\ \hline

\hline{2}{|c|}{Hacking Simulspace Systems} \\ \hline

â€“0 &View current status of simulspace, simulmorphs, and accessing egos \\ \hline

â€“10 &Change domain rules, add cheats, alter parameters of story, alter simulmorphs, change time dilation \\ \hline

â€“20 &Eject simulmorph, alter/erase character AIs \\ \hline

â€“30 &Abort simulation \\ \hline

\hline{2}{|c|}{Hacking Spimes} \\ \hline

â€“0 &Get status report, use device functions \\ \hline

â€“10 &Adjust AI/voice personality settings, adjust timed operation schedule \\ \hline

â€“20 &Disable sensors, disable device functions \\ \hline
...
This script has 5 steps, which Sarlo can handle with his Programming skill of 70. Sarlo succeeds in his Programming and Infosec Tests, and the script is loaded. It will then activate at the appropriate time. Since Sarloâ€™s account did not have access rights to perform these actions, each will require an Infosec Test using Sarloâ€™s skill to succeed. 
...
Jamming simply requires a Complex Action and an Interfacing Test. If successful, affected devices within range have their radio communications disruptedâ€” they are cut off from the mesh and may not communicate wirelessly. Wired devices are unaffected. 
...
Jamming can also be used to interfere with radar. In this case, if the jammer makes an Interface Test. If successful, the radar suffers interference, imposing a â€“30 modifier on all sensor-related tests. The entity operating the radar may attempt to overcome this interference by beating the jammer in an Opposed Interface Test. 
...
Anything goes in a simulspace, as dictated by the domain rules. A simulspace may range from approximating reality very closely to differing drastically Gravity might fluctuate, the visual light spectrum might not exist, characters might heal virtual damage effortlessly, simulmorphs may be capable of transmogrifying into other creatures, everything might be underwaterâ€”the possibilities are endless, limited only by imagination. In game terms, this allows the gamemaster to make up rules on the fly. 
...
â€“0 &Analyze simulation parameters, view domain rules, shape appearance of simulmorph, switch simulmorph character or morph type \\ \hline

â€“10 &Change probability of test outcomes, become invisible (â€œout-gameâ€?) to others \\ \hline

â€“20 &Interfere with simulation (e.g. make it rain, generate earthquakes), generate items, ignore domain rules, kill or lockout other simulmorphs \\ \hline

â€“30 &Go into god mode, command simulated characters, take over the simulation \\ \hline
...
AIs have aptitudes no greater than 20 but are incapable of defaulting. If they don't possess a skill, they don't know how to do it. (At the gamemaster's discretion, they may default to field skills or similar skills as noted on p. 173 with a â€“10 to â€“30 modifier). They can use skills like any character in \textit{Eclipse Phase}, however they may not possess any Active skill at a rating higher than 40 or Knowledge skill higher than 90â€”the maximum amount of expertise that their skill software allows. 

While AIs are programmed with personality templates and empathy, they are generally less emotive and difficult to read (apply a â€“30 modifier to Kinesics Test made against them, when in pod bodies). When combined with non-expressive synthetic morphs, they are even more difficult (â€“60 modifier). Some AIs lack emotive capabilities altogether and are impossible to read with Kinesics skill. 
